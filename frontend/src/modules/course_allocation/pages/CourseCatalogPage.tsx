  ...
  async function saveCatalog(entry: CatalogEntry) {
    try {
      await apiPutJson(`/course-allocation/catalog/${entry.id}`, {
        ...
        ug_type: entry.ug_type || null,
        pg_type: entry.pg_type || null,
      });
      setEditingId(null);
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-600 mt-1">
          Canonical course metadata — UG and PG each store their own type (Core / Elective). Edits propagate to matching
          allocation rows.
        </p>
      </div>
              ...
              return (
                <tr key={c.id} className="border-t border-slate-100">
                  <td className="px-3 py-2">
                    {editing ? (
                      <input
                        ...
                        onChange={(e) => setEditDraft((d) => ({ ...d, course_code: e.target.value }))}
                      />
                    ) : (
                      row.course_code
                    )}
                  ...
                  <td className="px-3 py-2">
                    {editing ? (
                      <input
                        ...
                        onChange={(e) => setEditDraft((d) => ({ ...d, course_name: e.target.value }))}
                      />
                    ) : (
                      row.course_name
                    )}
                  ...
                  <td className="px-3 py-2">
                    {editing ? (
                      <select
                        ...
                        value={row.ug_type ?? ""}
                        onChange={(e) => setEditDraft((d) => ({ ...d, ug_type: e.target.value || null }))}
                      >
                        {TYPE_OPTIONS.map((t) => (
                  ...
                  <td className="px-3 py-2">
                    {editing ? (
                      <select
                        className="border rounded px-2 py-1"
                        value={row.pg_type ?? ""}
                        onChange={(e) => setEditDraft((d) => ({ ...d, pg_type: e.target.value || null }))}
                      >
                        {TYPE_OPTIONS.map((t) => (
                          <option key={t || "none"} value={t}>
                            {t || "—"}
                  ...
                  <td className="px-3 py-2">
                    {editing ? (
                      <>
                        <button type="button" className="text-xs text-teal-800 mr-2" onClick={() => saveCatalog(row as CatalogEntry)}>
                          Save
                        </button>
                        <button
                          type="button"
